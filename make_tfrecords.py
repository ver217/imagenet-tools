# modified from https://cloud.google.com/tpu/docs/imagenet-setup
...
flags.DEFINE_string(
    'local_scratch_dir', './tf_records', 'Scratch directory path for temporary files.')
flags.DEFINE_string(
...
def _convert_to_example(image_buffer: str,
...
def convert_to_tf_records(
    ...
    validation_synsets = [os.path.basename(
        os.path.dirname(f)) for f in validation_files]
    validation_synsets = list(
        map(lambda x: bytes(x, 'utf-8'), validation_synsets))
